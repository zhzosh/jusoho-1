...
class Makingware_Tenpay_Model_Payment extends Mage_Payment_Model_Method_Abstract
{
	...
	{
		$url = $this->getConfigData('gateway');
	...
	public function getStandardCheckoutFormFields ()
	{
		...
		if (! ($order instanceof Mage_Sales_Model_Order)) {
			Mage::throwException(
			$this->_getHelper()->__('Cannot retrieve order object')
			);
		}
		...
		$parameter = array('cmdno' => 1, 'date' => $date,
			'bank_type' => 0,
			'desc' => Mage::getStoreConfig('design/head/default_title') . '-' .
			 $this->_getHelper()->__('Order id is :') . "{$transaction_id}",
			'bargainor_id' => $this->getConfigData('partner_id'),
