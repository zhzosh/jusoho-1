...
class Mage_Catalog_ProductController extends Mage_Core_Controller_Front_Action
{
    ...
    public function getProductMediaAction()
    {
        $model=Mage::getModel('catalog/product');
    	$product=$model->load($this->getRequest()->getParam('product_id'));
        $image=$product->getImage();
        
        if(empty($image) || $image=='no_selection'){
            $product=$model->load($this->getRequest()->getParam('base_product_id'));
        }
      
    	Mage::register('product', $product);
        $block = $this->getLayout()->createBlock('catalog/product_view_media');
        $block->setTemplate('catalog/product/view/media.phtml');
        echo $block->toHtml();
        die;
