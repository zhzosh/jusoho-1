...
class Mage_Catalog_Model_Product_Type_Configurable extends Mage_Catalog_Model_Product_Type_Abstract
{
    ...
    public function getConfigurableAttributesAsArray($product = null)
    {
        ...
        foreach ($this->getConfigurableAttributes($product) as $attribute) {       
            $res[] = array(
                ...
                'input_type'     => $attribute->getProductAttribute()->getFrontendInput(),
            );
        }  
        return $res;
    }
        ...
        if ($data = $this->getProduct($product)->getConfigurableAttributesData()) {
            foreach ($data as $attributeData) {
                ...
                Mage::getModel('catalog/product_type_configurable_attribute')
                   ...
                   ->save();
                //save color_text and color_pic
                if($attributeData['input_type']=='color'){
                    $colorTexts=$this->getProduct($product)->getConfigurableAttributesColorTextData();
                    $colorPicChecks=$this->getProduct($product)->getConfigurableAttributesColorPicCheckData();
                    $write = Mage::getSingleton('core/resource')->getConnection('core_write');
                    $colorTable=Mage::getSingleton('core/resource')->getTableName('eav_attribute_image_option_product_pic');
                      
                    foreach($attributeData['values'] as $key=>$value){
                        $dataVal=array('color_text'=>$colorTexts[$value['value_index']]);
                        $write->update($colorTable, $dataVal, array($write->quoteInto('option_id=?',$value['value_index']),
                            $write->quoteInto('product_id=?',$product->getId()),
                        ));
                        
                        if($_FILES['color_pic']['name'][$value['value_index']]){
                            $uploader = new Varien_File_Uploader(array('tmp_name'=>$_FILES['color_pic']['tmp_name'][$value['value_index']],
                                'name'=>$_FILES['color_pic']['name'][$value['value_index']]));    
                            $uploader->setAllowedExtensions(array('jpg','jpeg','gif','png')); //set allow format
                            ...
                            $imageUrl= $mediaModel->_moveImageFromTmp($imageTmp);//the path save to database
                            $dataVal=array('color_pic'=>$imageUrl);
                           $write->update($colorTable, $dataVal, array( $write->quoteInto('option_id=?',$value['value_index']),
                                $write->quoteInto('product_id=?',$product->getId()),
                             ));
                            
                            if(!empty($value['color_pic'])){
                                $this->deleteOldImage($value['color_pic']); //delete old image       
                            }      
                            
                        }else{
                            if($colorPicChecks[$value['value_index']]){
                                if(!empty($value['color_pic'])){
                                    $this->deleteOldImage($value['color_pic']); //delete old image
                                    $dataVal=array('color_pic'=>'');
                                  $write->update($colorTable, $dataVal, array($write->quoteInto('option_id=?',$value['value_index']),
                                    $write->quoteInto('product_id=?',$product->getId()),
                                  ));
                                }
    ...
    {
        $realPath=Mage::getSingleton('catalog/product_media_config')->getBaseMediaPath().$imagePath;
