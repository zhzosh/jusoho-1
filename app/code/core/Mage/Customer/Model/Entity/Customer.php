...
class Mage_Customer_Model_Entity_Customer extends Mage_Eav_Model_Entity_Abstract
{
    ...
    protected function _beforeSave(Varien_Object $customer)
    {
        ...
        $allowLoginTypes = Mage::getStoreConfig('customer/create_account/register_required_login_attributes');
        $allowLoginType = explode(',', $allowLoginTypes);
        $types = array('username' => 'validateUsername', 'phone' => 'validatePhone');
    ...
    protected function validateUsername(Varien_Object $customer)
    {
	        ...
	        if($itemId){
        		if ($customerId) {
        			if($itemId!=$customerId){
        				throw Mage::exception('Mage_Core', Mage::helper('customer')->__('This customer user name already exists.'),
     ...
     protected function validatePhone(Varien_Object $customer)
     {
        $phone=$customer->getPhone();

        if(!empty($phone)){
			$collection=Mage::getModel('customer/customer')->getResourceCollection();
	        $collection->addFieldToFilter('phone', $customer->getPhone());
	        $item=$collection->load()->getFirstItem();
	        $itemId=$item->getId();
	        $customerId=$customer->getId();
	        ...
	        if($itemId){
        		if ($customerId) {
        			if($itemId!=$customerId){
        				throw Mage::exception('Mage_Core', Mage::helper('customer')->__('This customer user phone already exists.'),
