 ...
 */
class Mage_Eav_Model_Mysql4_Entity_Attribute_Color extends Mage_Core_Model_Mysql4_Abstract
{
    public function _construct()
    {
        $this->_init('eav/attribute_image_option', 'option_id');
    }
    ...
    public function addOptionValueToCollection($collection, $attribute, $valueExpr) {
        ...
        $collection->getSelect()
            ->joinLeft(
                array($optionTable1 => $this->getTable('eav/attribute_image_option_value')),
                "`{$optionTable1}`.`option_id`={$valueExpr}"
                . " AND `{$optionTable1}`.`store_id`='0'",
                array())
            ->joinLeft(
                array($optionTable2 => $this->getTable('eav/attribute_image_option_value')),
    ...
    public function getFlatUpdateSelect(Mage_Eav_Model_Entity_Attribute_Abstract $attribute, $store, $hasValueField=true)
    {
        ...
        if (($attribute->getFrontend()->getInputType() != 'multiselect') && $hasValueField) {
            $select->joinLeft(
                array('to1' => $this->getTable('eav/attribute_image_option_value')),
                "`to1`.`option_id`={$valueExpr}"
                . " AND `to1`.`store_id`='0'",
                array())
            ->joinLeft(
                array('to2' => $this->getTable('eav/attribute_image_option_value')),
